...
public class Position
{
    ...
    public delegate void ChessDelegate(int x, int y);
    public event ChessDelegate Promotion;

    public delegate void AudioDelegate(int side);
    public event AudioDelegate StepDone;
    ...
    /// <param name="x">Координата по горизонтали</param>
    /// <param name="y">Координата по вертикали</param>
    public void MakeStep(int index, int x, int y, bool realStep = false)
    {
        if (realStep)
            StepDone(Pieces[index].Side);
        ...
        if (piece.Name == "King")
        {
            if (x - x0 == 2)
            {
                if (piece.Side == 1)
                {
                    ...
                    if (realStep)
                        Coordinates.MovePiece(8, 1, 6, 1);
                }
                else
                {
                    Pieces[19].PositionX = 6;
                    if (realStep)
                        Coordinates.MovePiece(8, 8, 6, 8);
                }
            }
            if (x - x0 == -2)
            {
                if (piece.Side == 1)
                {
                    ...
                    if (realStep)
                        Coordinates.MovePiece(1, 1, 4, 1);
                }
                else
                {
                    Pieces[18].PositionX = 4;
                    if (realStep)
                        Coordinates.MovePiece(1, 8, 4, 8);
                }
            }
        }
        ...
        if (realStep)
            Coordinates.MovePiece(x0, y0, x, y);
